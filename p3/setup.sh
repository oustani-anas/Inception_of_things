...
sudo kubectl create namespace argocd || echo "Namespace argocd exists"
sudo kubectl create namespace dev || echo "Namespace dev exists"
...
sudo kubectl apply -n dev -f - <<EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  name: wil-playground
...
sudo kubectl wait --for=condition=Ready pod -l app=wil-playground -n dev --timeout=60s

# -----------------------------
# 5️⃣ Port-forward dev app
# -----------------------------
echo "Port-forward dev app to host..."
sudo kubectl port-forward --address 0.0.0.0 svc/wil-playground-service -n dev 8888:8888 &
